...
The `hud2007`--`hud2025` data frames hold HUD's annual Point-in-Time (PIT)
estimate of the total ("Overall Homeless") population for each Continuum of Care
(CoC). The `coc2007`--`coc2025` objects hold the matching CoC boundaries as
`sf` polygons.
...
`pit_coc_detail` carries HUD's gender breakdown, available 2014--2024 (dropped
from the 2025 release). National totals by gender, summing across CoCs:

```{r gender-coc, eval = have_map}
